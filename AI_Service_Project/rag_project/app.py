...
<style>
    ...
    .main-header .sub { margin: 0.4rem 0 0; opacity: 0.75; font-size: 0.9rem; }
    .main-header .warn {
        margin: 0.8rem 0 0;
        font-size: 0.78rem;
        background: #e2b96f22;
    ...
    .case-card:hover { border-color: #e2b96f88; }
    .case-card .tag {
    ...
    .case-card p { margin: 0; font-size: 0.85rem; color: #cbd5e1; }
    ...
    .stage-log .stage-title { color: #93c5fd; font-weight: 600; }
    .stage-log .stage-detail { color: #94a3b8; margin-top: 0.2rem; }
    ...
    for msg in st.session_state.chat_history:
        with st.chat_message(msg["role"]):
            ...
            if msg["role"] == "assistant":
                ...
                if used is True:
                    st.markdown('<span style="font-size:0.75rem;color:#4ade80">📖 법률 문서 기반 (Self-RAG)</span>', unsafe_allow_html=True)
                elif used is False:
                    st.markdown('<span style="font-size:0.75rem;color:#fb923c">⚡ 일반 지식 기반</span>', unsafe_allow_html=True)
...
with col_right:
    st.markdown("#### 🔍 Self-RAG 처리 단계")

    if st.session_state.process_log:
        for stage, detail in st.session_state.process_log:
            st.markdown(f"""
<div class="stage-log">
...
""", unsafe_allow_html=True)
        ...
        st.info("질문하면 Self-RAG의 단계별 처리 과정이 여기에 표시됩니다.")

    st.divider()
    st.markdown("#### 📊 Self-RAG 파이프라인")
    st.markdown("""
```
사용자 사건 설명 / 질문
         ↓
